use crate::transfer::common::err::TransferErrorType::{MessageTypeNotAcceptedError, NoTypeFieldError, ValidationError};
use crate::transfer::schemas::{TRANSFER_COMPLETION_SCHEMA, TRANSFER_REQUEST_SCHEMA, TRANSFER_START_SCHEMA, TRANSFER_SUSPENSION_SCHEMA, TRANSFER_TERMINATION_SCHEMA};
use anyhow::bail;
...
pub async fn schema_validation(json_value: Value) -> anyhow::Result<()> {
    ...
    let validation = match message_type.unwrap() {
        ...
        "dspace:TransferTerminationMessage" => TRANSFER_TERMINATION_SCHEMA.apply(&json_value).basic(),
        _ => {
