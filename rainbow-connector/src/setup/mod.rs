...
impl ConnectorSetup {
    ...
    pub async fn get_connector_repo(&self, config: &CatalogConfig) -> Arc<dyn ConnectorRepoTrait> {
        ...
        let dataplane_repo = Arc::new(ConnectorRepoForSql::create_repo(db_connection));
        dataplane_repo
    }
    pub async fn build_control_router(&self, config: &CatalogConfig) -> Router {
        let dataplane_repo = self.get_connector_repo(config).await;
        ...
        Router::new().nest("/templates", connector_template_router)
    }
}
