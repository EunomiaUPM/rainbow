# ADR-03-2: Bypass Proxy & Peer Discovery (RPC)
...
In a distributed agent ecosystem, agents often need to consume catalogs from peers directly—a "Bypass" scenario where no central federator is involved. This presents a challenge: the agent must resolve a peer's physical location and specific protocol version solely from their decentralized identity (DID). Furthermore, querying remote agents over the public internet introduces latency that can degrade the user experience of the local agent.
