    ...
    V: CatalogOdrlFacadeTrait + Send + Sync,
{
    ...
    async fn post_request(&self, input: ContractRequestMessage) -> anyhow::Result<ContractAckMessage> {
        ...
        let resolver = self.catalog_facade.resolve_odrl_offers(odrl_ids).await;
        if resolver.is_err() {
            bail!(IdsaCNError::NotCheckedError { provider_pid: None, consumer_pid: None, error: "Id not found".to_string() })
        }
        let offer = resolver?;

        // If target, control target is correct
        match &input.odrl_offer {
            ContractRequestMessageOfferTypes::OfferMessage(input_offer) => {
                if input_offer.target.clone() != offer.target.clone().unwrap() {
                    bail!(IdsaCNError::NotCheckedError { provider_pid: None, consumer_pid: None, error: "target not coincide".to_string() })
                }
            }
    ...
    async fn post_provider_request(
        ...
        // Check if consumer_pid exists in the database
        // all this in middleware
        let _ = self.repo.get_cn_processes_by_consumer_id(input.consumer_pid.clone()).await?.ok_or(
        ...
        let cn_process =
            self.repo.get_cn_processes_by_provider_id(&provider_pid).await.map_err(IdsaCNError::DbErr)?.ok_or(
                IdsaCNError::ProcessNotFound {
    ...
    async fn post_provider_agreement_verification(
        ...
        let ContractAgreementVerificationMessage { provider_pid, consumer_pid, _type, .. } = input.clone();
        let cn_process =
            self.repo.get_cn_processes_by_provider_id(&provider_id).await.map_err(IdsaCNError::DbErr)?.ok_or(
                IdsaCNError::ProcessNotFound {
