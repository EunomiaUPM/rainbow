...
pub struct ContractNegotiationConsumerMigration;

impl MigratorTrait for ContractNegotiationConsumerMigration {
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        let mut migrations: Vec<Box<dyn MigrationTrait>> = vec![];
        let mut contract_negotation_migrations = get_contracts_migrations();
        let mut pub_sub_migrations = get_events_migrations();
        migrations.append(&mut contract_negotation_migrations);
