...
use rainbow_contracts::provider::http::rainbow_idsa_triggers::{SetupAgreementRequest, SetupAgreementResponse, SetupFinalizationRequest, SetupFinalizationResponse, SetupOfferRequest, SetupOfferResponse};
...
use rainbow_contracts::consumer::http::rainbow_idsa_triggers::{SetupAcceptanceRequest, SetupAcceptanceResponse, SetupRequestRequest, SetupRequestResponse, SetupVerificationRequest, SetupVerificationResponse};
...
#[traced_test]
#[tokio::test]
pub async fn contract_negotiation_provider() -> anyhow::Result<()> {
    ///
    /// Setup servers
    ///
    ...
    let provider_pid = res.provider_pid.unwrap();
    let consumer_pid = res.consumer_pid.unwrap();

    // -------------------------------
    // Consumer contraoffers with REQUEST
    // -------------------------------
    ...
    // -------------------------------
    let req = consumer_client
        .post("http://127.0.0.1:1235/api/v1/negotiations/rpc/setup-acceptance")
        .json(&SetupAcceptanceRequest {
