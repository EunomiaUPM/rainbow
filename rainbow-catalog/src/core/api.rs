...
use uuid::Uuid;
use crate::data::entities::dataset::Model;
...
pub async fn dataset_request(dataset_id: Uuid) -> anyhow::Result<Dataset> {
    ...
    match datasets_from_db {
        Some(dataset_from_db) => {
            ...
            dataset.distribution = distributions_request_by_dataset(dataset.id.parse()?, dataset_from_db.catalog_id).await?;
            Ok(dataset)
        }
...
pub async fn dataset_request_by_catalog(catalog_id: Uuid) -> anyhow::Result<Vec<Dataset>> {
    ...
    for dataset_entity in datasets_from_db {
        ...
        let dataset_odrl_from_db = odrl_offer::Entity::find()
            ...
            .await?;
        dataset.odrl_offer = to_value(dataset_odrl_from_db)?;
        dataset.distribution = distributions_request_by_dataset(dataset.id.parse()?, catalog_id).await?;
...
pub async fn distributions_request_by_dataset(dataset_id: Uuid, catalog_id: Uuid) -> anyhow::Result<Vec<Distribution>> {
    let db_connection = db_connection().await?;
    ...
    for distribution_entity in distributions_from_db {
        ...
        // dataservice
        distribution.access_service = dataservices_request_by_catalog(catalog_id).await?;
...
pub async fn catalog_request() -> anyhow::Result<Vec<Catalog>> {
    ...
    let catalogs_from_db = catalog::Entity::find()
        .limit(5)
        .all(&db_connection)
        .await?;
