...
where
    T: RainbowRPCDatahubCatalogTrait + Send + Sync + 'static,
{
    service: Arc<T>,
}

impl<T> RainbowRPCDatahubCatalogRouter<T>
where
    T: RainbowRPCDatahubCatalogTrait + Send + Sync + 'static,
{
    ...
    pub fn router(self) -> Router {
        Router::new()
            .route(
                "/api/v1/datahub/rpc/resolve-data-service",
                post(Self::handle_resolve_data_service),
            )
            .route(
                "/api/v1/datahub/rpc/resolve-offer",
    ...
    pub async fn handle_resolve_data_service(
        ...
        input: Result<Json<RainbowRPCDatahubCatalogResolveDataServiceRequest>, JsonRejection>, // Todo define object
    ) -> impl IntoResponse {
        info!("POST /api/v1/datahub/rpc/resolve-data-service");
    ...
    pub async fn handle_resolve_offer(
        State(service): State<Arc<T>>,
        input: Result<Json<RainbowRPCDatahubCatalogResolveOfferByIdRequest>, JsonRejection>, // Todo define object
    ) -> impl IntoResponse {
        info!("POST /api/v1/datahub/rpc/resolve-offer");
