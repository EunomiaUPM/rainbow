 ...
 *
 */
use crate::data::entities::auth_verification::{Column, Entity, Model, NewModel};
use crate::errors::{ErrorLogTrait, Errors};
use crate::services::repo::repos::GenericRepo;
use crate::services::repo::traits::{AuthVerificationRepoTrait, BasicRepoTrait, IntoActiveSet};
use anyhow::bail;
...
#[async_trait]
impl BasicRepoTrait<Model, NewModel> for AuthVerificationRepo {
...
#[async_trait]
impl AuthVerificationRepoTrait for AuthVerificationRepo {
    async fn get_by_state(&self, state: &str) -> anyhow::Result<Model> {
        let model = match Entity::find().filter(Column::State.eq(state)).one(&self.inner.db_connection).await {
            Ok(Some(data)) => data,
            Ok(None) => {
    ...
    //     let model = match active_model.insert(&self.inner.db_connection).await {
    //         Ok(data) => data,
    //         Err(e) => {
    //             let error = Errors::database_new(e.to_string());
    //             error!("{}", error.log());
