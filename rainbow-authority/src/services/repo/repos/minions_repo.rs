...
use crate::services::repo::traits::{BasicRepoTrait, IntoActiveSet, MinionsRepoTrait};
use anyhow::bail;
...
impl MinionsRepoTrait for MinionsRepo {
    async fn get_me(&self) -> anyhow::Result<Model> {
        let me = match Entity::find().filter(Column::IsMe.eq(true)).one(&self.inner.db_connection).await {
            Ok(Some(data)) => data,
            Ok(None) => {
                let error = Errors::missing_resource_new(0.to_string(), "Missing resource myself".to_string());
                error!("{}", error.log());
                bail!(error)
            }
            Err(e) => {
