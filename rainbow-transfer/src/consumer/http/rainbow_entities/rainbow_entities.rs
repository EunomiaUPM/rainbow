    ...
    T: RainbowTransferConsumerServiceTrait + Send + Sync + 'static,
{
    ...
    pub fn router(self) -> Router {
        Router::new()
            ...
            .route("/api/v1/transfers/:id", get(Self::handle_get_transfer_by_id))
            .route("/api/v1/transfers", post(Self::handle_post_transfer))
            .route("/api/v1/transfers/:id", put(Self::handle_put_transfer_by_id))
            .route("/api/v1/transfers/:id", delete(Self::handle_delete_transfer_by_id))
            .with_state(self.transfer_service)
    }
    async fn handle_get_all_transfers(
    ...
    async fn handle_post_transfer(
        ...
        let input = match input {
            Ok(input) => input.0,
            Err(e) => return DSRPCTransferConsumerErrors::DSProtocolTransferConsumerError(DSProtocolTransferConsumerErrors::JsonRejection(e.body_text())).into_response(),
        };
