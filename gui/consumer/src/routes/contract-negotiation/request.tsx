...
export const RouteComponent = () => {
  ...
  const [consumerSelectedParticipant, setConsumerSelectedParticipant] =
  ...
  const handleCatalogOpenChange = async (newOpenState: boolean) => {
    ...
    if (newOpenState) {
      try {
        const fetchedCatalogs = await getBypassCatalogs(
          api_gateway,
          consumerSelectedParticipant?.participant_id as UUID,
        );
  ...
  const handleDatasetOpenChange = async (newOpenState: boolean) => {
    ...
    if (newOpenState && selectedCatalog) {
      ...
      try {
        const fetchedDatasets = await getBypassDatasetsByCatalogId(
          api_gateway,
          consumerSelectedParticipant?.participant_id as UUID,
          selectedCatalog["@id"],
        );
  ...
  const handlePoliciesOpenChange = async (newOpenState: boolean) => {
    ...
    if (newOpenState && selectedDataset) {
      ...
      try {
        const fetchedPolicies = await getBypassPoliciesByDatasetId(
          api_gateway,
          consumerSelectedParticipant?.participant_id as UUID,
          selectedDataset["@id"],
        );
              ...
              <FormItem>
                ...
                <div>
                  ...
                  <FormDescription className="text-sm text-gray-500 mt-1">
                    Provide the ID of the provider participant for the negotiation.
                  </FormDescription>
                  <FormMessage />
                </div>
              </FormItem>
...
export const Route = createFileRoute("/contract-negotiation/request")({
