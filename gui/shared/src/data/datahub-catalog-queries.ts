...
export const getDatahubCatalogs = async (api_gateway: string) => {
    const catalogs: DatahubDomain[] = await (
        await fetch(api_gateway + "/datahub/domains")
    ).json();
    return catalogs;
}

export const getDatahubCatalogsOptions = (api_gateway: string) => queryOptions({
...
export const useGetDatahubCatalogs = () => {
    ...
    const {data, isLoading, isError, error} = useSuspenseQuery(getDatahubCatalogsOptions(api_gateway))
...
export const getDatahubDatasetsByCatalogIdptions = (api_gateway: string, urn: string) => queryOptions({
...
export const useGetDatahubDatasetsByCatalogId = (urn: string) => {
    const {api_gateway} = useContext<GlobalInfoContextType>(GlobalInfoContext);
    const {data, isLoading, isError, error} = useSuspenseQuery(getDatahubDatasetsByCatalogIdptions(api_gateway, urn))
    return {data, isLoading, isError, error}
}
