...
export const getParticipants = async (api_gateway: string) => {
    const catalogs: Participant[] = await (
        await fetch(api_gateway + "/mates")
    ).json();
    return catalogs;
}

export const getParticipantsOptions = (api_gateway: string) => queryOptions({
...
export const getParticipantById = async (api_gateway: string, participantId: UUID) => {
    const catalogs: Participant = await (
        await fetch(api_gateway + `/mates/${participantId}`)
    ).json();
    return catalogs;
}

export const getParticipantByIdOptions = (api_gateway: string, participantId: UUID) => queryOptions({
...
export const useGetParticipantById = (participantId: UUID) => {
    ...
    const {
        ...
        isError,
        error
    } = useSuspenseQuery(getParticipantByIdOptions(api_gateway, participantId))
    return {data, isLoading, isError, error}
}
...
export const getAgreementsByParticipantIdOptions = (api_gateway: string, participantId: UUID) => queryOptions({
...
export const useGetAgreementsByParticipantId = (participantId: UUID) => {
    ...
    const {
        ...
        isError,
        error
    } = useSuspenseQuery(getAgreementsByParticipantIdOptions(api_gateway, participantId))
    return {data, isLoading, isError, error}
}
