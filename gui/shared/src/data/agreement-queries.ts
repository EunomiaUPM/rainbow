...
export const getAgreementsOptions = (api_gateway: string) => queryOptions({
    queryKey: ["AGREEMENTS"],
...
export const getAgreementByIdOptions = (api_gateway: string, agreementId: UUID) => queryOptions({
...
export const useGetAgreementById = (agreementId: UUID) => {
    const {api_gateway} = useContext<GlobalInfoContextType>(GlobalInfoContext);
    const {data, isLoading, isError, error} = useSuspenseQuery(getAgreementByIdOptions(api_gateway, agreementId))
    return {data, isLoading, isError, error}
}
