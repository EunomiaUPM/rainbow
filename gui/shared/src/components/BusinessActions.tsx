...
const BusinessActionsForProvider = ({process}: {
    process: CNProcess
}) => {
    return (
...
const BusinessActionsForConsumer = ({process}: {
    process: CNProcess
}) => {
    return (<div className="space-x-2 min-w-[260px]">
        {process.state == "REQUESTED" ? (
            <>
                <Dialog>
                    <DialogTrigger asChild>
...
export const BusinessActions = ({process}: {
    process: CNProcess
}) => {
    const {participant} = useContext<AuthContextType | null>(AuthContext)!
    return (
        <div className="inline-flex items-center">
            {participant?.participant_type == "Provider" && <BusinessActionsForProvider process={process}/>}
            {participant?.participant_type == "Consumer" && <BusinessActionsForConsumer process={process}/>}
        </div>
    );
};
