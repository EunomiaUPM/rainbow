...
import {Form, FormControl, FormDescription, FormField, FormItem, FormMessage} from "shared/src/components/ui/form";
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <FormField control={form.control} name="method" render={({field}) => (
                            <FormItem>
                                <label htmlFor="method" className="text-sm mb-1 text-inherit">Method</label>
                                <FormControl>
                                    <Select {...field} defaultValue={transferMethods[0]} onValueChange={field.onChange}>
                                        <SelectTrigger className="w-[240px]">
                                            <SelectValue placeholder="Select a transfer protocol"/>
                                        </SelectTrigger>
                                        <SelectContent>
                                            {transferMethods.map(transferMethod => (
                                                <SelectItem value={transferMethod}
                                                            key={transferMethod}>{transferMethod}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </FormControl>
                        ...
                        <FormField control={form.control} name="transfer_protocol" render={({field}) => (
                            <FormItem>
                                <label htmlFor="transfer_protocol" className="text-sm mb-1">Transfer protocol</label>
                                <FormControl>
                                    <Select {...field} defaultValue={transferProtocols[0]}
                                        ...
                                        <SelectContent>
                                            {transferProtocols.map(transferProtocol => (
                                                <SelectItem value={transferProtocol}
                                                            key={transferProtocol}>{transferProtocol}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </FormControl>
