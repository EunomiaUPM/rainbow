...
use sea_orm::{
    ActiveModelTrait, ActiveValue, ColumnTrait, DatabaseConnection, EntityTrait, JoinType, QueryFilter, QueryOrder,
...
impl AgreementRepo for ContractNegotiationProviderRepoForSql {
    ...
    async fn get_agreements_by_participant_id(
        ...
        let agreement = agreement::Entity::find()
            .filter(
                Condition::any()
                    .add(agreement::Column::ProviderParticipantId.eq(participant_id.clone()))
                    .add(agreement::Column::ConsumerParticipantId.eq(participant_id.clone())),
            )
            .all(&self.db_connection)
            .await
            .map_err(|err| CnErrors::ErrorFetchingAgreement(err.into()))?;
