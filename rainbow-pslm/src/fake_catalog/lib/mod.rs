...
pub fn create_dataset(endpoint: String, endpoint_properties: Option<&Value>) -> anyhow::Result<DatasetsCatalogModel> {
    let transaction = create_dataset_repo(endpoint, endpoint_properties)?;
    Ok(transaction)
}
