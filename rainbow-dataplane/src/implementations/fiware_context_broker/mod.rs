...
use crate::core::{
    DataPlanePeer, DataPlanePeerCreationBehavior, PersistModel,
};
    ...
    async fn persist(self, db_connection: &DbConn) -> anyhow::Result<Box<Self>> {
        ...
        if let Some(dp) = dp {
            ...
            for attribute in attributes {
                let exists = &self.inner.attributes
                    .iter()
                    .any(|attr| attr.0.to_string() == attribute.key && attr.1.to_string() == attribute.value);
                if !exists {
