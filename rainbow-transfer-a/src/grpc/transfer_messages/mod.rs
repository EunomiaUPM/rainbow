...
use crate::grpc::api::transfer_messages::{CreateMessageRequest, PaginationRequestMessages, ResourceIdRequestMessages, TransferMessageListResponse, TransferMessageResponse};
...
impl TransferAgentMessages for TransferAgentMessagesGrpc {
    async fn get_all_messages(&self, request: Request<PaginationRequestMessages>) -> Result<Response<TransferMessageListResponse>, Status> {
        let proto_req = request.into_inner();
        let params: PaginationParams = proto_req.into();
        let messages = self.service
    ...
    async fn create_message(&self, request: Request<CreateMessageRequest>) -> Result<Response<TransferMessageResponse>, Status> {
        todo!()
    }

    async fn get_message_by_id(&self, request: Request<ResourceIdRequestMessages>) -> Result<Response<TransferMessageResponse>, Status> {
        todo!()
    }
    ...
    async fn get_messages_by_process_id(&self, request: Request<ResourceIdRequestMessages>) -> Result<Response<TransferMessageListResponse>, Status> {
        todo!()
    }
}