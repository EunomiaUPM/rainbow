...
use super::config::{VerifierConfig, VerifierConfigTrait};
...
use crate::ssi::common::types::vcs::{VPDef, VerifyPayload};
use anyhow::bail;
...
use crate::ssi::common::data::entities::{recv_interaction, recv_verification};
use crate::ssi::common::utils::{get_claim, get_opt_claim, split_did};
...
impl VerifierService {
    pub fn new(client: Arc<dyn ClientServiceTrait>, config: VerifierConfig) -> VerifierService {
        VerifierService { client, config }
    }
}
...
impl VerifierTrait for VerifierService {
    ...
    fn generate_uri(&self, ver_model: &recv_verification::Model) -> String {
        ...
        let presentation_definition_uri = format!("{}/pd/{}", &host, ver_model.state);
        let encoded_presentation_definition_uri = encode(&presentation_definition_uri);

        let response_uri = format!("{}/verify/{}", &host, ver_model.state);
    ...
    fn get_vpd(&self, model: &recv_verification::Model) -> VPDef {
    ...
    fn validate_nonce(&self, model: &recv_verification::Model, token: &TokenData<Value>) -> anyhow::Result<()> {
        ...
        if model.nonce != nonce {
            let error = AuthErrors::security_new("Invalid nonce, it does not match");
            error!("{}", error.log());
            bail!(error);
        }
    ...
    fn validate_holder(&self, model: &recv_verification::Model, token: &TokenData<Value>) -> anyhow::Result<()> {
        ...
        if model.holder.clone().unwrap() != vp_holder {
            // EXPECTED ALWAYS
            let error = AuthErrors::security_new("Invalid holder, it does not match");
            error!("{}", error.log());
            bail!(error);
        }
