...
mod tests {
    ...
    use rainbow_common::ssi::{ClientConfig, SSIWalletConfig};
    ...
    async fn test_join_exchange_server_error() {
        ...
        let server_address = server.address();
        let config: ApplicationConsumerConfig = ApplicationConsumerConfig {
            transfer_process_host: None,
            business_system_host: None,
            contract_negotiation_host: None,
            catalog_bypass_host: None,
        ...
        let error_msg = format!("{:?}", result.unwrap_err());
        assert!(
            error_msg.contains("500") || error_msg.contains("Internal Server Error"),
            "Mensaje de error inesperado: {}",
            error_msg
        );
 ...
 #[tokio::test]
    async fn test_present_vp_server_error() {
        ...
        let error_msg = format!("{:?}", result.unwrap_err());
        assert!(
            error_msg.contains("error decoding") || error_msg.contains("500") || error_msg.contains("Petition"),
            "Mensaje de error inesperado: {}",
            error_msg
        );
