...
mod tests {
    ...
    impl BasicRepoTrait<Model, NewModel> for MockMatesRepo {
        ...
        async fn create(&self, _: NewModel) -> Result<Model> {
    ...
    impl MatesRepoTrait for MockMatesRepo {
        ...
        async fn force_create(&self, _: NewModel) -> Result<Model> {
            Err(anyhow::anyhow!("Not implemented"))
        }
    }
    ...
    impl MockManager {
        ...
        async fn login_wallet(&self) -> Result<(), rainbow_common::errors::CommonErrors> {
    ...
    async fn verify_handler(State(m): State<Arc<MockManager>>) -> impl IntoResponse {
        match m.verify_all("state123".to_string(), "vp_token".to_string()).await {
    ...
    async fn test_all_routes_success() {
        ...
        for route in [
            ...
            let method = if route.contains("verify")
                ...
                || route.contains("business")
                || route.contains("retrieve")
            {
                "POST"
            } else {
                "GET"
    ...
    async fn test_access_request_success() {
        ...
        let payload = rainbow_auth::ssi_auth::common::types::gnap::GrantRequest {
    ...
    async fn test_register_key_and_did_success() {
        ...
        for route in ["/api/v1/wallet/key", "/api/v1/wallet/did"] {
            let response = router
                .clone()
                .oneshot(Request::builder().method("POST").uri(route).body(Body::empty()).unwrap())
                .await
                .unwrap();
    ...
    async fn test_register_and_delete_key_did_error() {
        ...
        for (route, method, payload) in [
            ...
            println!(
                "Register/Delete error route: {}, status: {:?}",
                route,
                response.status()
            );
    ...
    async fn test_retrieve_business_mate_token_error() {
        ...
        let payload = RainbowBusinessLoginRequest { auth_request_id: "invalid_id".to_string() };
        let body = serde_json::to_vec(&payload).unwrap();
