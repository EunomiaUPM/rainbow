...
impl NegotiationAgentAgreementsTrait for NegotiationAgentAgreementsService {
    async fn get_all_agreements(&self, limit: Option<u64>, page: Option<u64>) -> anyhow::Result<Vec<AgreementDto>> {
        let agreements =
            self.negotiation_repo.get_agreement_repo().get_all_agreements(limit, page).await.map_err(|e| {
    ...
    async fn get_batch_agreements(&self, ids: &Vec<Urn>) -> anyhow::Result<Vec<AgreementDto>> {
        let agreements = self.negotiation_repo.get_agreement_repo().get_batch_agreements(ids).await.map_err(|e| {
            let err = CommonErrors::database_new(&e.to_string());
            error!("{}", err.log());
            err
        })?;
    ...
    async fn get_agreement_by_id(&self, id: &Urn) -> anyhow::Result<Option<AgreementDto>> {
        let agreement = self.negotiation_repo.get_agreement_repo().get_agreement_by_id(id).await.map_err(|e| {
            let err = CommonErrors::database_new(&e.to_string());
            error!("{}", err.log());
            err
        })?;
    ...
    async fn get_agreement_by_negotiation_process(&self, id: &Urn) -> anyhow::Result<Option<AgreementDto>> {
        let agreement =
            self.negotiation_repo.get_agreement_repo().get_agreement_by_negotiation_process(id).await.map_err(|e| {
                let err = CommonErrors::database_new(&e.to_string());
                error!("{}", err.log());
                err
            })?;
    ...
    async fn get_agreement_by_negotiation_message(&self, id: &Urn) -> anyhow::Result<Option<AgreementDto>> {
        let agreement =
            self.negotiation_repo.get_agreement_repo().get_agreement_by_negotiation_message(id).await.map_err(|e| {
                let err = CommonErrors::database_new(&e.to_string());
                error!("{}", err.log());
                err
            })?;
    ...
    async fn create_agreement(&self, new_model_dto: &NewAgreementDto) -> anyhow::Result<AgreementDto> {
        let new_model: NewAgreementModel = new_model_dto.clone().into();

        let created = self.negotiation_repo.get_agreement_repo().create_agreement(&new_model).await.map_err(|e| {
            let err = CommonErrors::database_new(&e.to_string());
            error!("{}", err.log());
            err
        })?;
    ...
    async fn put_agreement(&self, id: &Urn, edit_model_dto: &EditAgreementDto) -> anyhow::Result<AgreementDto> {
        let edit_model: EditAgreementModel = edit_model_dto.clone().into();

        let updated =
            self.negotiation_repo.get_agreement_repo().put_agreement(id, &edit_model).await.map_err(|e| match e {
                AgreementRepoErrors::AgreementNotFound => {
                    let err = CommonErrors::missing_resource_new(&id.to_string(), "Agreement not found for update");
                    error!("{}", err.log());
                    err
                }
                _ => {
    ...
    async fn delete_agreement(&self, id: &Urn) -> anyhow::Result<()> {
        self.negotiation_repo.get_agreement_repo().delete_agreement(id).await.map_err(|e| match e {
            AgreementRepoErrors::AgreementNotFound => {
                let err = CommonErrors::missing_resource_new(&id.to_string(), "Agreement not found for deletion");
                error!("{}", err.log());
                err
            }
            _ => {
